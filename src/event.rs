...
use crate::payment::recurrence::{
	record_failure, record_success, RecurrenceAttempt, RecurrenceFailure, RecurrenceManager,
	RecurrenceStatus,
};
	...
	L::Target: LdkLogger,
{
	...
	/// Queues one durable lifecycle event when a recurrence status actually changes.
	///
	/// The transition ID is emitted with the status so consumers can distinguish replay of an old
	/// event from a later transition that happens to use the same status value.
	...
	/// Converts an LDK payment failure into durable recurrence state.
	///
	/// The current payment window is calculated from the persisted offer and basetime so an
	/// expired window becomes `Missed`, while failures before its close remain retryable.
...
mod tests {
	...
	#[test]
	/// Verify that a queued event written with the pre-migration layout remains readable.
	fn event_queue_reads_legacy_layout_fixture() {
		let fixture = hex_utils::to_vec(
			"0001033400201717171717171717171717171717171717171717171717171717171717171717021000000000000000000000000000000913",
		)
		.unwrap();
		let store: Arc<DynStore> = Arc::new(DynStoreWrapper(InMemoryStore::new()));
		let logger = Arc::new(TestLogger::new());
		let event_queue = EventQueue::read(&mut &fixture[..], (store, logger)).unwrap();

		assert_eq!(
			event_queue.next_event(),
			Some(Event::ChannelReady {
				channel_id: ChannelId([23; 32]),
				user_channel_id: UserChannelId(2323),
				counterparty_node_id: None,
				funding_txo: None,
			})
