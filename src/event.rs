					...
					Err(err) => {
						...
						self.channel_manager
							...
							.unwrap_or_else(|e| {
								...
								debug_assert!(
									false, "Failed to force close channel after funding generation failed"
								);
							});
					},
				}
...
mod tests {
	...
	///
	/// Reading the fixed bytes checks that the old queue format still keeps the event available after
	/// a restart.
