...
mod tests {
	...
	#[test]
	/// Verify that an address-pool record written with the old format remains readable.
	///
	/// The fixed bytes contain several address numbers, so the test checks that the whole pool is
	/// restored rather than only its first entry.
