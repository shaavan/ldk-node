...
pub struct Node {
	...
	recurrence_manager: Arc<crate::payment::recurrence::RecurrenceManager>,
...
impl Node {
	...
	fn start_inner(&self, is_running_lock: &mut bool) -> Result<(), Error> {
		...
		let event_handler = Arc::new(EventHandler::new(
			...
			Arc::clone(&self.recurrence_manager),
...
mod tests {
	...
	///
	/// The test sends fixed identifiers through the real event handler, then checks the node's queue
	/// to confirm applications receive the cancellation event they can handle.
	async fn recurring_offer_cancelled_is_queued_as_incoming_payment_cancelled() {
		...
		let node = NodeBuilder::new().build_with_store(node_entropy, InMemoryStore::new()).unwrap();
		let bump_tx_event_handler = Arc::new(BumpTransactionEventHandler::new(
